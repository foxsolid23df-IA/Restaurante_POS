    ...
    <section className="bg-white rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden">
      <div className="p-10 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-50/30 backdrop-blur-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-2xl font-black text-slate-900 tracking-tight">Inventario Maestro</h3>
            <p className="text-slate-500 font-medium text-sm">Detalle técnico de existencias por unidad</p>
          </div>
        </div>
        
        <div className="flex bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm">
           <div className="px-6 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-3">
              <Package size={14}/>
              Items: {items.length}
           </div>
        </div>
                ...
                <tr key={item.id} className="hover:bg-slate-50/80 transition-all group duration-300">
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-black text-slate-900 group-hover:text-primary transition-colors text-lg tracking-tight uppercase">{item.name}</p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Categoría: Suministro</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-10 py-8 text-center">
                    <div className="flex flex-col items-center">
                      <span className={`text-2xl font-black tracking-tighter ${isCritical ? 'text-rose-600' : 'text-slate-900'}`}>
                        {item.current_stock}
                      </span>
                  ...
                  <td className="px-10 py-8 text-center">
                    <span className="text-sm font-black text-slate-300 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
                  ...
                  <td className="px-10 py-8 text-right">
                    <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:translate-x-0 translate-x-4">
                      ...
                      <button 
                        ...
                        className="p-4 bg-white text-rose-500 rounded-[1.2rem] border-2 border-rose-50 hover:border-rose-500 hover:bg-rose-50 transition-all"
                        title="Eliminar Item"
                      >
                        <Trash2 size={18} />
                      </button>
      ...
      {items.length === 0 && (
        <div className="p-20 text-center">
            ...
            <h4 className="text-2xl font-black text-slate-900 mb-2 tracking-tight">No se encontraron items</h4>
            <p className="text-slate-400 font-medium">Comienza agregando insumos a tu base de datos central</p>
        </div>
      )}
    </section>
  )
